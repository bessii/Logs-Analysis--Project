...
request_error_days = \
    """
    select day, percentage from (select day, round((sum(views)/(select
    count(*) from log where substring(cast(log.time as text), 0, 11) =
    day) * 100), 2) as percentage from (select substring(cast(log.time as
    text), 0, 11) as day, count(*) as views from log where status like
    '%404%' group by day) as log_percentage group by day order by percentage
    desc) as final where percentage >= 1.0; """
